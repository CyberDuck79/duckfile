...
func TestRenderTemplateDelimsAndAllowMissing(t *testing.T) {
	withTempWD(t, func() {
		...
		if err := renderTemplate(src, dst, targ, targ.Template, map[string]any{"FOO": "X"}); err != nil {
			t.Fatal(err)
		}
		b, _ := os.ReadFile(dst)
		if string(b) != "X {{ .BAR }}" {
...
func TestRenderTemplateInvalidSyntax(t *testing.T) {
	withTempWD(t, func() {
		...
		err := renderTemplate(src, dst, targ, targ.Template, map[string]any{"FOO": "x"})
...
func TestEnsureRenderedLogic(t *testing.T) {
	withTempWD(t, func() {
		...
		p, err := computeTemplatePaths("t", testTarget, testTarget.Template, vars)
		if err != nil {
		...
		if err := ensureRendered(false, false, targ, targ.Template, vars, p); err != nil {
		...
		if err := ensureRendered(true, false, targ, targ.Template, vars, p); err != nil {
